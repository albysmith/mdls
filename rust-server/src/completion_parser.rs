...
pub fn simple_complete(params: CompletionParams) -> Vec<CompletionItem> {
    ...
    let (byte_position, string) = get_node_and_string(params);
                            ...
                            match f_child.tag_name().name() {
                                "actions" => {
                                    for actions_child in f_child.children() {
                                        info!("actions_child {}", actions_child.tag_name().name());
...
fn get_node_and_string(params: CompletionParams) -> (usize, String) {
